...
> - Stable 镜像 release ID：`0.2.0`（`v0.3.0` 不重建镜像）
...
Strix Halo 使用统一物理内存，宿主内核驱动、容器设备权限和 ROCm 用户态必须正确配合。直接在宿主机反复安装 ROCm，或让每个 Python venv 各自安装数 GB 的 Torch wheel，容易造成版本漂移、磁盘浪费和难以恢复的环境。
...
脚本不能修改 BIOS。工具不会安装 `amd-debug-tools`、调用 `amd-ttm`、写入 `ttm.conf`、设置 `ttm.pages_limit` 或 `amdgpu.gttsize`，也不会要求 GTT/TTM 重启。现有 GTT/TTM 值只作为只读诊断事实记录，不参与宿主写入或通过判定。
...
1. **完整工作站安装**：检查并准备 Ubuntu OEM 6.17 内核、旧 ROCm 残留、Docker、Buildx 和设备组，然后部署镜像与第一个项目。
...
完整模式先展示独立的内核计划。只有精确输入 `INSTALL-KERNEL` 才会应用内核变更；重启并通过内核检查后，再展示独立的平台计划，精确输入 `APPLY` 才会继续。主要动作包括：

- 备份待修改文件和主机状态到 `/var/backups/amd-ai/<UTC时间戳>/`；
- 安全禁用确认属于 ROCm 6.4 的源并清理确认的旧包；
- 安装并启动 Ubuntu OEM 6.17 系列内核，保留当前内核作为桌面恢复入口；
- 安装匹配 headers、firmware 和宿主诊断工具；
- Docker 缺失时安装 Docker Engine、Buildx 和 Compose 插件；
- 根据真实 `/dev/kfd` 和 render node GID 配置目标用户设备组。

平台阶段不会修改 GTT/TTM，也不会触发第二次重启。工具只读取 live TTM 页数、内核命令行和相关日志用于诊断。
...
重启后进入同一个 checkout，再次执行原命令。安装器核对 boot ID，验证当前 OEM 6.17 内核、显示管理器、`amdgpu`、KFD/render node 和当前启动日志，再进入平台计划；不会重复已经完成的内核写入。

若新启动的 OEM 6.17 patch kernel 高于最低版本，且设备权限、GPU 设备和内核日志检查通过，但内核尚未进入静态已测清单，会显示 `WARN HOST_VERIFY unverified` 并继续部署。状态文件记录验证状态、内核版本和诊断码；真正的 `change-required` 或 `blocked` 仍会停止。观察到的 TTM 值不会产生 mismatch 阻断。
...
适用于 Docker、OEM `amdgpu` 和设备权限已经准备好的宿主：
...
省略 `--state-path` 时安装器根据规范化后的项目绝对路径自动选择独立状态文件。`v0.3.0` 在 `PLAN` 中显示 `状态=<路径>（per-project）`；不同项目不会误用第一个项目的 `full` 状态。
...
无交互完整模式分别审批内核计划和平台计划。先用完全相同的目标用户、项目目录和项目名运行一次，在 `INSTALL-KERNEL` 提示处拒绝写入；退出码 `2` 是预期行为，内核计划摘要已经写入状态：
...
KERNEL_PLAN_DIGEST="$(
  STATE="$STATE" python3.12 -c \
    'import json, os; print(json.load(open(os.environ["STATE"], encoding="utf-8"))["kernel_plan_digest"])'
)"
printf 'accepted kernel plan: %s\n' "$KERNEL_PLAN_DIGEST"
```

使用该摘要启动无交互安装。内核应用后安装器停在 `KERNEL_REBOOT_PENDING`；手工重启，再原样执行同一命令。内核验证通过后，安装器会生成平台计划，并因尚未提供平台摘要而停止：
...
./install.sh \
  ...
  --project-name video-lab \
  --image-source pull \
  --accept-kernel-plan-digest "$KERNEL_PLAN_DIGEST" \
  --state-path "$STATE"
```

从状态中读取并审阅新生成的平台摘要，然后完成安装：
...
./install.sh \
  ...
  --image-source pull \
  --accept-kernel-plan-digest "$KERNEL_PLAN_DIGEST" \
  --accept-host-plan-digest "$HOST_PLAN_DIGEST" \
  --accept-docker-group \
  --state-path "$STATE"
```

`--accept-docker-group` 可以省略，此时不授权 Docker 组。如果任一计划在两次运行之间变化，对应 digest 校验会阻断应用并要求重新审阅。无交互模式不会自动接受计划变化、digest 漂移、build fallback 或重启。
...
`v0.3.0` 的安装输出是确定的逐行事件，不使用 spinner，也不会依赖 TTY 原地刷新。默认顺序为 `PLAN`、`LOG`，然后每个选定阶段显示 `SKIP` 或 `START`；长步骤可显示 `DETAIL`、子命令原始进度和每 15 秒一次的 `WAIT`，检查点安全写入后才显示 `PASS`，最后显示 `SUMMARY` 和同一条 `LOG` 路径。
...
控制目录权限为 `0700`，日志文件为 `0600`。`v0.3.0` 不自动轮转或删除日志；确认不再需要审计证据后，由该用户自行清理。日志包含 Docker pull、BuildKit `plain` 输出、uv/pip、锁定 wheel 下载进度和 sudo helper 的 stderr；JSON 协议 stdout、检查命令和摘要读取仍保持捕获模式。
...
再次选择相同模式和项目目录。v0.3.0 使用状态 schema 3 和新的 17 阶段 full 流程。可重建旧 `BOOTSTRAP` 摘要的 `0.2.x` container 状态可以接管；full 状态会从 `BOOTSTRAP` 重新评估，但保留不可变镜像身份，且绝不复用旧宿主审批。升级后必须重新审阅独立的内核与平台计划。摘要损坏或输入漂移时必须按失败输出修复；不要删除状态文件绕过检查。
...
当前没有自动宿主回滚/卸载命令。只有确认安装器不处于待重启/待恢复状态，并且不再需要状态证据时，才移除用户 launcher 和运行时。该操作不会恢复内核或用户组；工具从不写入 GTT/TTM：
...
项目目录、模型和 Docker 镜像需由用户分别确认后处理。宿主修改应根据 `/var/backups/amd-ai/` 的备份按[宿主运维手册](docs/host-operations.md)恢复；不要把删除 OEM 内核或恢复软件源简化为通用卸载脚本。现有 TTM 配置不是本工具创建或管理的内容。
...
| `strix-halo-rocm host-verify` | 验证设备权限、GPU 和内核日志，并只读记录实时 GTT/TTM 事实 |
